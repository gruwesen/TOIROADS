...
class TorchDataSetMaker():
    ...
    def generate_torch_dataset_with_congestion(number, min_size, max_size, capacity_factor=1, density=0.04):
        """
        ...
        """
        dataset = []
        for network in GraphMaker.generate_graphs(number, min_size, max_size, capacity_factor, density):
...
class GraphMaker():
    ...
    @staticmethod
    def generate_graphs(number, min_size, max_size, capacity_factor=1, density=0.04):
        for i in range(number):
            r = RoadMaker()
...
class RoadMaker():
    ...
    def make_network(self, min_size=4, max_size=10):
        self.network_size = randint(min_size,max_size)
        self.network = []
        self.totpop = 0
        self.totpoi = 0
        for i in range(self.network_size):
            road_length = randint(1,10)
            population = max(0,randint(-3,2))
            poi = max(0,randint(-4,2))
            #Capacity feature is experimental and some unused functions in RGG ignores its existence.
            capacity = randint(self.network_size//2, self.network_size)
